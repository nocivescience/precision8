...
const warnings=[
  ...
  x: warning.column * (wallW + pathW) + (wallW / 2 + pathW / 2-10),
  y: warning.row * (wallW + pathW) + (wallW / 2 + pathW / 2-10) 
}))
    ...
    setInterval(function(){
      warnings.forEach(({x,y})=>{
        balls.forEach(({x:bx,y:by})=>{
          if(Math.abs(x-bx)<12 && Math.abs(y-by)<12){
            changingBg=true;
          }else {
            changingBg=false;
          }
        ...
        warning.textContent = `(${x},${y})`;
        warning.style.cssText = `
            background-color: ${changingBg ? "rgba(0, 183, 255, 0.599)" : "rgba(247, 0, 255, 0.599)"};
            text-align: center;
            display: flex;
            ...
            user-select: none;
          `;
        mazeElement.appendChild(warning);
      })
    }, 100);
...
window.addEventListener("keydown", function (event) {
  ...
  if (![" ", "H", "h", "E", "e", "J", "j"].includes(event.key)) return;
